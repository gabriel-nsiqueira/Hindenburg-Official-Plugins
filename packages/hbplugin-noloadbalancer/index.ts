...
export default class NoLoadBalancerPlugin {
    ...
    createFakeClient(client: Client) { // Clone the client for the fake worker.
        ...
        fakeClient.mods = client.mods ? client.mods.slice(0) : undefined;
    ...
    @OnMessage(AcknowledgePacket)
    async onAcknowledgementPacket(message: AcknowledgePacket, direction: MessageDirection, sender: Client) {
        if (!this.server.isLoadBalancer())
            return;
        ...
        for (const sent of fakeWorkerClient.sent) {
